  ...
  afterCreate(instance, server) {
    instance.identifiers.forEach(identifier => {
      ...
      if (!identifierTypeId) {
        ...
        //if (!issn) {
          let issn = server.create('identifier-type', { name: 'issn' });
        //}
    ...
    instance.contributors.forEach(contributor => {
      ...
      if (!contributorNameTypeId) {
        ...
        //if (!type) {
          let type = server.create('contributor-name-type', { name: 'Personal name' });
        //}
    ...
    afterCreate(instance, server) {
      ...
      instance.notes.forEach(note => {
        ...
        if (!instanceNoteTypeId) {
          ...
          //if (!type) {
            let type = server.create('instance-note-type', {
              name: 'General note',
              source: 'folio',
              metadata: {
